/**
 * ===================================================================
 * RV암기 - 요한계시록 본문 데이터 (개역한글 기준)
 * ===================================================================
 *
 * 저작권 안내:
 * 개역한글 성경 본문은 대한성서공회가 저작권을 가지고 있어
 * 이 파일에 본문을 미리 채워둘 수 없습니다.
 * 아래 형식대로 각 장의 절을 문자열 배열로 직접 입력해 주세요.
 *
 * 입력 방법:
 * 1. 가지고 계신 개역한글 성경 앱/사이트에서 원하는 장의 본문을 복사합니다.
 * 2. 절(verse) 단위로 끊어서 아래 배열의 빈 문자열 자리에 붙여넣습니다.
 *    예) 1장 1절을 입력하려면:
 *    1: [
 ...
 *      ...
 *    ],
 * 3. 절 앞의 숫자(1, 2, 3...)는 적지 않고 본문 내용만 넣습니다.
 * 4. 각 절은 쉼표로 구분된 별도의 문자열(" ")이어야 합니다.
 * 5. 한 장의 절을 모두 채운 뒤 다음 장으로 넘어가면 됩니다.
 * 6. 장 전체를 비워두면(빈 배열 []) 앱에서 "본문 없음"으로 표시되며
 *    해당 장은 단어시험/백지시험 범위 선택에서 자동으로 비활성화됩니다.
 *
 * 주의: 개역한글은 개역개정과 띄어쓰기·어휘가 다른 부분이 있습니다
 * (예: "되신"/"되신", "효되" 등 표기 차이, "임이라"/"이라" 등 어미 차이).
 * 백지시험은 띄어쓰기까지 정확히 비교하므로, 반드시 개역한글 원문 그대로
 * 정확하게 입력해 주세요.
 *
 * 작성이 끝나면 이 파일을 index.html과 같은 폴더에 두고 열면 됩니다.
 * ===================================================================
 */
...
"귀 있는 자는 성령이 교회들에게 하시는 말씀을 들을찌어다"
  ...
  3: ["사데 교회의 사자에게 편지하기를 하나님의 일곱 영과 일곱 별을 가진이가 가라사대 내가 네 행위를 아노니 네가 살았다 하는 이름은 가졌으나 죽은 자로다",
...
"귀 있는 자는 성령이 교회들에게 하시는 말씀을 들을찌어다",
  ...
  4: ["이 일 후에 내가 보니 하늘에 열린 문이 있는데 내가 들은바 처음에 내게 말하던 나팔소리 같은 그 음성이 가로되 이리로 올라오라 이 후에 마땅히 될 일을 내가 네게 보이리라 하시더라",
...
"네 생물이 가로되 아멘 하고 장로들은 엎드려 경배하더라",
  ...
  6: ["내가 보매 어린 양이 일곱 인 중에 하나를 떼시는 그 때에 내가 들으니 네 생물 중에 하나가 우뢰소리 같이 말하되 오라 하기로",
...
"이는 보좌 가운데 계신 어린 양이 저희의 목자가 되사 생명수 샘으로 인도하시고 하나님께서 저희 눈에서 모든 눈물을 씻어 주실 것임이러라",
  ...
  8: ["일곱째 인을 떼실 때에 하늘이 반시 동안쯤 고요하더니",
...
"또 그 살인과 복술과 음행과 도적질을 회개치 아니하더라",
  ...
  10: ["내가 또 보니 힘센 다른 천사가 구름을 입고 하늘에서 내려 오는데 그 머리 위에 무지개가 있고 그 얼굴은 해 같고 그 발은 불기둥 같으며",
...
"이에 하늘에 있는 하나님의 성전이 열리니 성전 안에 하나님의 언약궤가 보이며 또 번개와 음성들과 뇌성과 지진과 큰 우박이 있더라",
  ...
  12: ["하늘에 큰 이적이 보이니 해를 입은 한 여자가 있는데 그 발 아래는 달이 있고 그 머리에는 열 두 별의 면류관을 썼더라",
...
"지혜가 여기 있으니 총명 있는 자는 그 짐승의 수를 세어 보라 그 수는 사람의 수니 육백 육십 륙이니라",
  ...
  14: ["또 내가 보니 보라 어린 양이 시온산에 섰고 그와 함께 십 사만 사천이 섰는데 그 이마에 어린 양의 이름과 그 아버지의 이름을 쓴 것이 있도다",
...
"성 밖에서 그 틀이 밟히니 틀에서 피가 나서 말굴레까지 닿았고 일천 육백 스다디온에 퍼졌더라",
  ...
  15: ["또 하늘에 크고 이상한 다른 이적을 보매 일곱 천사가 일곱 재앙을 가졌으니 곧 마지막 재앙이라 하나님의 진노가 이것으로 마치리로다",
...
"하나님의 영광과 능력을 인하여 성전에 연기가 차게 되매 일곱 천사의 일곱 재앙이 마치기까지는 성전에 능히 들어갈 자가 없더라",
  ...
  16: ["또 내가 들으니 성전에서 큰 음성이 나서 일곱 천사에게 말하되 너희는 가서 하나님의 진노의 일곱 대접을 땅에 쏟으라 하더라",
...
"또 중수가 한 달란트나 되는 큰 우박이 하늘로부터 사람들에게 내리매 사람들이 그 박재로 인하여 하나님을 훼방하니 그 재앙이 심히 큼이러라",
  ...
  17: ["또 일곱 대접을 가진 일곱 천사 중 하나가 와서 내게 말하여 가로되 이리 오라 많은 물위에 앉은 큰 음녀의 받을 심판을 네게 보이리라",
...
"또 네가 본바 여자는 땅의 임금들을 다스리는 큰 성이라 하더라",
  ...
  18: ["이 일 후에 다른 천사가 하늘에서 내려오는 것을 보니 큰 권세를 가졌는데 그의 영광으로 땅이 환하여지더라",
...
"선지자들과 성도들과 및 땅 위에서 죽임을 당한 모든 자의 피가 이 성중에서 보였느니라 하더라",
  ...
  19: ["이 일 후에 내가 들으니 하늘에 허다한 무리의 큰 음성 같은 것이 있어 가로되 할렐루야 구원과 영광과 능력이 우리 하나님께 있도다",
...
// 장별 제목(소제목) - 선택 화면에 표시됩니다. 필요에 맞게 수정하셔도 됩니다.
const CHAPTER_TITLES = {
  1: "1~8 계시록 전장의 요약과 결론, 9~20 계시록 사건의 시작과 일곱 별과 일곱 금 촛대의 비밀",
  2: "일곱 교회 사자에게 보낸 편지 - 에베소·서머나·버가모·두아디라 교회",
  3: "일곱 교회 사자에게 보낸 편지 - 사데·빌라델비아·라오디게아 교회",
  4: "영계 하나님의 보좌와 계열",
  5: "일곱 인으로 봉한 책",
  6: "배도한 선천 해·달·별ㅔ 대한 심판",
